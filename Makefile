# This Makefile is for the Time::HiRes extension to perl.
#
...
#     NAME => q[Time::HiRes]
#     PREREQ_PM => { Carp=>q[0], Config=>q[0], Exporter=>q[0], ExtUtils::MakeMaker=>q[0], Test::More=>q[0], XSLoader=>q[0], strict=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[HiRes.pm]
#     XSOPT => q[]
#     clean => { FILES=>q[xdefine] }
#     dist => { CI=>q[ci -l], COMPRESS=>q[gzip -9f], SUFFIX=>q[gz] }
...
# These definitions are from config.sh (via /Users/nicolas/workspace/perl5/lib/Config.pm).
...
SITEARCHEXP = /usr/local/lib/perl5/site_perl/5.29.8/darwin-2level
SO = dylib
VENDORARCHEXP = 
VENDORLIBEXP = 
...
INSTALLARCHLIB = /usr/local/lib/perl5/5.29.8/darwin-2level
DESTINSTALLARCHLIB = $(DESTDIR)$(INSTALLARCHLIB)
INSTALLSITEARCH = /usr/local/lib/perl5/site_perl/5.29.8/darwin-2level
...
PERL = "../../miniperl"
FULLPERL = "../../perl"
ABSPERL = /Users/nicolas/workspace/perl5/dist/Time-HiRes/../../miniperl
...
MAKEMAKER   = /Users/nicolas/workspace/perl5/cpan/ExtUtils-MakeMaker/lib/ExtUtils/MakeMaker.pm
MM_VERSION  = 7.34
MM_REVISION = 73400
...
INC = 
DEFINE = -DTIME_HIRES_NANOSLEEP -DTIME_HIRES_CLOCKID_T -DTIME_HIRES_CLOCK_GETTIME -DTIME_HIRES_CLOCK_GETRES -DTIME_HIRES_CLOCK_NANOSLEEP -DTIME_HIRES_CLOCK_NANOSLEEP_EMULATION -DTIME_HIRES_CLOCK -DHAS_FUTIMENS -DHAS_UTIMENSAT -DTIME_HIRES_UTIME -DTIME_HIRES_STAT_ST_XTIMESPEC -DTIME_HIRES_STAT=1 -DATLEASTFIVEOHOHFIVE
...
INST_LIBDIR      = $(INST_LIB)/Time
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Time
...
XSUBPPARGS =  -typemap '/Users/nicolas/workspace/perl5/dist/Time-HiRes/../../lib/ExtUtils/typemap' -typemap '/Users/nicolas/workspace/perl5/dist/Time-HiRes/typemap'
XSUBPP_EXTRA_ARGS =
...
CCFLAGS = -fno-common -DPERL_DARWIN -no-cpp-precomp -mmacosx-version-min=10.14 -fno-strict-aliasing -pipe -fstack-protector-strong -I/usr/local/include -DPERL_USE_SAFE_PUTENV -Wall -Werror=declaration-after-statement -Werror=pointer-arith -Wextra -Wc++-compat -Wwrite-strings
OPTIMIZE = -O3
PERLTYPE = 
MPOLLUTE = 
...
PASTHRU = LIBPERL_A="$(LIBPERL_A)"\
	...
	OPTIMIZE="$(OPTIMIZE)"\
	PREFIX="$(PREFIX)"\
	PASTHRU_DEFINE='-DTIME_HIRES_NANOSLEEP -DTIME_HIRES_CLOCKID_T -DTIME_HIRES_CLOCK_GETTIME -DTIME_HIRES_CLOCK_GETRES -DTIME_HIRES_CLOCK_NANOSLEEP -DTIME_HIRES_CLOCK_NANOSLEEP_EMULATION -DTIME_HIRES_CLOCK -DHAS_FUTIMENS -DHAS_UTIMENSAT -DTIME_HIRES_UTIME -DTIME_HIRES_STAT_ST_XTIMESPEC -DTIME_HIRES_STAT=1 -DATLEASTFIVEOHOHFIVE $(PASTHRU_DEFINE)'\
...
.c.i:
	ccache gcc  -no-cpp-precomp -E -c $(PASTHRU_INC) $(INC) \
	$(CCFLAGS) $(OPTIMIZE) \
	$(PERLTYPE) $(MPOLLUTE) $(DEFINE_VERSION) \
...
clean :: clean_subdirs
	- $(RM_F) \
	  ...
	  *$(LIB_EXT) *$(OBJ_EXT) \
	  *perl.core HiRes.base \
	  HiRes.bs HiRes.bso \
	  HiRes.c HiRes.def \
	  HiRes.exp HiRes.o \
	  HiRes_def.old MYMETA.json \
...
realclean purge :: realclean_subdirs
	- $(RM_F) \
	  $(FIRST_MAKEFILE) $(INST_LIB)/Time/HiRes.pm \
	  $(MAKEFILE_OLD) $(OBJECT) 
	- $(RM_RF) \
	  $(DISTVNAME) $(INST_ARCHAUTODIR) \
	  $(INST_AUTODIR) const-c.inc \
	  const-xs.inc 
...
test_static :: subdirs-test_static $(MAP_TARGET)
	PERL_DL_NONLAZY=1 "/Users/nicolas/workspace/perl5/dist/Time-HiRes/$(MAP_TARGET)" $(MAP_PERLINC) "-MExtUtils::Command::MM" "-MTest::Harness" "-e" "undef *Test::Harness::Switches; test_harness($(TEST_VERBOSE), '$(INST_LIB)', '$(INST_ARCHLIB)')" $(TEST_FILES)

testdb_static :: static pure_all $(MAP_TARGET)
	PERL_DL_NONLAZY=1 "/Users/nicolas/workspace/perl5/dist/Time-HiRes/$(MAP_TARGET)" $(MAP_PERLINC) "-I$(INST_LIB)" "-I$(INST_ARCHLIB)" $(TEST_FILE)
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Time-HiRes" VERSION="1.9760">' > Time-HiRes.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>High resolution alarm, sleep, gettimeofday, interval timers</ABSTRACT>' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Jarkko Hietaniemi &lt;jhi@iki.fi&gt;</AUTHOR>' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Carp::" />' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Config::" />' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Exporter::" />' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="ExtUtils::MakeMaker" />' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::More" />' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="XSLoader::" />' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="strict::" />' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="darwin-2level-5.29" />' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Time-HiRes.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Time-HiRes.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'HiRes.pm' '$(INST_LIB)/Time/HiRes.pm' 
